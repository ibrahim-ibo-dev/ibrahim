...
export default function Experience() {
  ...
  useEffect(() => {
    ...
    cards.forEach((card, i) => {
      ...
      const cardAnim = gsap.fromTo(
        card,
        { opacity: 0, x: isLeft ? -60 : 60, rotateY: isLeft ? 8 : -8 },
        {
          opacity: 1,
          x: 0,
          rotateY: 0,
  ...
  return (
    <section
      id="experience"
      ref={sectionRef}
      className="py-32 relative overflow-hidden exp-beams"
    >
          ...
          <div className="space-y-16">
            {experiences.map((exp, i) => {
              ...
              return (
                <div
                  key={i}
                  className={`exp-card relative grid md:grid-cols-2 gap-8 items-center`}
                  style={{ opacity: 0, perspective: "1000px" }}
                >
                  {/* Timeline dot */}
                  <div className="absolute left-1/2 top-8 -translate-x-1/2 z-10 hidden md:flex flex-col items-center">
                    <div
                  ...
                  <motion.div
                    whileHover={{ y: -4 }}
                    className={`glass-card rounded-2xl p-6 border ${exp.borderColor} ${isLeft ? "md:col-start-1" : "md:col-start-2"
                      }`}
                  >
                  ...
                  <div
                    className={`hidden md:flex items-start pt-6 ${isLeft ? "md:col-start-2 justify-start pl-12" : "md:col-start-1 md:row-start-1 justify-end pr-12"
                      }`}
                  >
                    <span
